...
export async function POST(request: NextRequest) {
  try {
    ...
    const expectedHeaders = ['교환권명', '금액', '조합원ID', '영농회', '이름', '생년월일', '휴대폰', '메모'];
    ...
    // 필수 컬럼 확인
    const requiredHeaders = ['교환권명', '금액', '조합원ID', '영농회', '이름', '생년월일', '휴대폰'];
      ...
      try {
        ...
        headers.forEach((header: string, index: number) => {
          ...
          if (mappedKey) {
            ...
            // 데이터 타입 변환
            if (mappedKey === 'amount') {
              value = typeof value === 'number' ? value : parseInt(String(value).replace(/[^\d]/g, ''), 10);
            } else if (mappedKey === 'dob') {
    ...
    console.log(`엑셀 파일 처리 완료: ${previewData.length}개 데이터, ${errors.length}개 오류`);

    return NextResponse.json({
      success: true,
      message: `${previewData.length}개의 데이터를 성공적으로 불러왔습니다.`,
      previewData,
